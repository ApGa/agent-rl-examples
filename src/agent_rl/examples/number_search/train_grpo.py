...
def main(args):
    config, _ = load_expr_config(args, NumberSearchGRPOConfig)
    config: NumberSearchGRPOConfig
    ...
    train_dataset = Dataset.from_list([get_task(x) for x in train_ids])
    ...
    valid_dataset = Dataset.from_list([get_task(x) for x in valid_ids])
    ...
    for global_step in range(start_step, max_steps):
        ...
        with stats_tracker.record_timing("rollout"):
            ...
            print(f"Batch rewards: {batch['rewards']}")
        ...
        stats[0].update(stats_tracker.export_all(reduce_group=actor.data_parallel_group))
