...
class NumberSearch:
    ...
    def _sample_bounds_around_target(
        self, target: int, rng: random.Random | None = None
    ) -> tuple[int, int]:
...
def load_task_from_disk(id: str) -> dict:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_samples", type=int, default=50000, help="Number of training samples to generate"
    )
    parser.add_argument(
        "--eval_size", type=int, default=1000, help="Number of validation/test samples to generate"
    )
    args = parser.parse_args()

    train_data, val_data = create_number_search_datasets(
